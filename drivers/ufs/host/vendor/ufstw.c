...
static int ufstw_check_lifetime_not_guarantee(struct ufstw_lu *tw)
{
	...
	if (tw->lifetime_est & MASK_UFSTW_LIFETIME_NOT_GUARANTEE) {
		if (tw->lun == TW_LU_SHARED)
			WARN_MSG("lun-shared lifetime_est[%d] (1)",
				UFSTW_LIFETIME_NOT_GURANTEE_BIT);
		else
			WARN_MSG("lun %d lifetime_est[%d] (1)",
				tw->lun, UFSTW_LIFETIME_NOT_GURANTEE_BIT);
...
static void ufstw_interval_work_fn(struct work_struct *work)
{
	...
	ufshcd_rpm_get_sync(tw->ufsf->hba);
...
put_sync_out:
	ufshcd_rpm_put_sync(tw->ufsf->hba);
...
void ufstw_prep_fn(struct ufsf_feature *ufsf, struct ufshcd_lrb *lrbp)
{
	...
	tw->write_lifetime += blk_rq_sectors(req);
	...
	tw->write_resize += blk_rq_sectors(req);
...
void ufstw_get_dev_info(struct ufsf_feature *ufsf, u8 *desc_buf)
{
	...
	INFO_MSG("tw_dev [0x%.2X] bTurboWriteBufferNoUserSpaceReductionEn (%u)",
		 DEVICE_DESC_PARAM_TW_RETURN_TO_USER,
		 tw_dev_info->tw_buf_no_reduct);
	INFO_MSG("tw_dev [0x%.2X] bTurboWriteBufferType (%u)",
		 DEVICE_DESC_PARAM_TW_BUF_TYPE, tw_dev_info->tw_buf_type);
	INFO_MSG("tw_dev [0x%.2X] dNumSharedTUrboWriteBufferAllocUnits (%u)",
		 DEVICE_DESC_PARAM_TW_SHARED_BUF_ALLOC_UNITS,
...
void ufstw_get_geo_info(struct ufsf_feature *ufsf, u8 *geo_buf)
{
	...
	INFO_MSG("tw_geo [0x%.2X] dTurboWriteBufferMaxNAllocUnits (%u)",
		 GEOMETRY_DESC_TW_MAX_SIZE,
		 get_unaligned_be32(geo_buf + GEOMETRY_DESC_TW_MAX_SIZE));
	INFO_MSG("tw_geo [0x%.2X] bDeviceMaxTurboWriteLUs (%u)",
		 GEOMETRY_DESC_TW_NUMBER_LU, tw_dev_info->tw_number_lu);
	INFO_MSG("tw_geo [0x%.2X] bTurboWriteBufferCapAdjFac (%u)",
		 GEOMETRY_DESC_TW_CAP_ADJ_FAC,
		 geo_buf[GEOMETRY_DESC_TW_CAP_ADJ_FAC]);
	INFO_MSG("tw_geo [0x%.2X] bSupportedTWBufferUserSpaceReductionTypes (%u)",
		 GEOMETRY_DESC_TW_SUPPORT_USER_REDUCTION_TYPES,
		 geo_buf[GEOMETRY_DESC_TW_SUPPORT_USER_REDUCTION_TYPES]);
	INFO_MSG("tw_geo [0x%.2X] bSupportedTurboWriteBufferTypes (%u)",
		 GEOMETRY_DESC_TW_SUPPORT_BUF_TYPE,
...
static void ufstw_get_lu_info(struct ufsf_feature *ufsf, int lun, u8 *lu_buf)
{
	...
	if (lu_desc.tw_lu_buf_size) {
		...
		INFO_MSG("ufstw_lu[%d] [0x%.2X] dLUNumTWBufferAllocUnits (%u)",
			 lun, UNIT_DESC_TW_LU_WRITE_BUFFER_ALLOC_UNIT,
