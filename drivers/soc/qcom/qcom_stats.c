				...
				struct kobj_attribute *attr, char *buf)
{
	...
	for (i = 0; i < n_subsystems; i++) {
		...
		for (j = 0; j < ARRAY_SIZE(subsystems); j++) {
			if (!strcmp(subsystems[j].name, name)) {
				//Begin George  WIFI ONLY,has not modem subsystem
				if((get_project() == 23926 || get_project() == 23927 || get_project() == 23976 || get_project() == 23978) && (!strcmp(subsystems[j].name, "modem"))){
				   break;
				}
				//End   George
